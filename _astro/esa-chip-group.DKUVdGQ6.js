      ...
      <div class="root" @keydown=${this.onKeydown}>
        ${(this.options??[]).map((e,r)=>{const t=this.isActive(e),s=this.multiple?r===0:t;return o`
            <button
              ...
              class="chip chip--${e.tone??"neutral"} ${t?"chip--active":""} typography-${d[this.size]}"
              part="chip"
              tabindex=${s?0:-1}
              aria-checked=${t}
              @click=${()=>this.select(e)}
