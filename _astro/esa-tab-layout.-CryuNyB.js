import{i,b as r,t as n,a as l}from"./typography.C7xuE2z_.js";const d={xs:"microcopy-xs-subtle",sm:"microcopy-sm-subtle",md:"microcopy-md-subtle",lg:"microcopy-lg-subtle"},c={xs:"microcopy-xs",sm:"microcopy-sm",md:"microcopy-md",lg:"microcopy-lg"};class b extends i{constructor(){super(),this.onKeydown=(e,t)=>{let a=null;switch(e.key){case"ArrowRight":a=this.findNextEnabledTab(t,1);break;case"ArrowLeft":a=this.findNextEnabledTab(t,-1);break;case"Home":a=this.findNextEnabledTab(-1,1);break;case"End":a=this.findNextEnabledTab(this.tabs.length,-1);break;default:return}a!==null&&(e.preventDefault(),this.selectTab(a),e.target.parentElement?.children[a]?.focus())},this.tabs=[],this.activeIndex=0,this.size="md",this.variant="underline",this.appearance="underline"}static{this.properties={tabs:{type:Array},activeIndex:{type:Number,attribute:"active-index"},size:{type:String,reflect:!0},variant:{type:String,reflect:!0},appearance:{type:String,reflect:!0}}}selectTab(e){this.tabs[e]?.disabled||(this.activeIndex=e,this.dispatchEvent(new CustomEvent("tabchange",{detail:{index:e},bubbles:!0,composed:!0})))}findNextEnabledTab(e,t){let a=e+t;for(;a>=0&&a<this.tabs.length;){if(!this.tabs[a].disabled)return a;a+=t}return null}render(){return r`
      <div class="layout">
